...
class GuesserBot(PyBot):
    def setup(self, setup):
        p = setup['punter']

        possible_claims = []
        for river in setup['map']['rivers']:
            src, trg = river['source'], river['target']
            if src > trg:
                src, trg = trg, src
            assert src != trg, 'Bad river {}'.format(river)
            possible_claims.append((src, trg))
...
if __name__ == '__main__':
    names = 'Christeen Berta Jamee Moira Hermila Gennie Un Malissa Ja Annabel Shalanda Freida Madlyn Agnes Mandy Tona Tess Dolores  Danuta Epifania'.split()
    bot = GuesserBot(random.choice(names))
    bot.run()
